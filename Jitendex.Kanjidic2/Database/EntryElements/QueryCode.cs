...
internal static class QueryCodeData
{
    ...
    private const string C5 = nameof(QueryCode.Misclassification);
    ...
    private const string InsertSql =
        $"""
        INSERT INTO "{nameof(QueryCode)}"
        ("{C1}", "{C2}", "{C3}", "{C4}", "{C5}") VALUES
        ( {P1} ,  {P2} ,  {P3} ,  {P4} ,  {P5} );
        """;

    public static async Task InsertQueryCodesAsync(this Context db, List<QueryCode> queryCodes)
    {
        await using var command = db.Database.GetDbConnection().CreateCommand();
        command.CommandText = InsertSql;

        foreach (var queryCode in queryCodes)
        {
