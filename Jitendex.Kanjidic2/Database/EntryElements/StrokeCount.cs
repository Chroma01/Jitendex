...
internal static class StrokeCountData
{
    ...
    private const string InsertSql =
        $"""
        INSERT INTO "{nameof(StrokeCount)}"
        ("{C1}", "{C2}", "{C3}") VALUES
        ( {P1} ,  {P2} ,  {P3} );
        """;

    public static async Task InsertStrokeCountsAsync(this Context db, List<StrokeCount> strokeCounts)
    {
        await using var command = db.Database.GetDbConnection().CreateCommand();
        command.CommandText = InsertSql;

        foreach (var strokeCount in strokeCounts)
        {
